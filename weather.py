...
def retrieve_report(url):
    try:
        ...
        wstats = {}
        wstats["curr_temp"] = soup.find(class_="today_nowcard-temp").find("span").text
        ...
        wstats["low_temp"], wstats["high_temp"] = sorted([e.find("span").text for e in soup.find_all(class_="deg-hilo-nowcard")])
        if wstats["low_temp"] == "--":
